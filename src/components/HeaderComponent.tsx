...
const HeaderComponent = () => {
  ...
  const handleEmailCopy = () => {
    ...
    copyConfirmation?.classList.replace("opacity-0", "opacity-100");
    copyConfirmation?.classList.toggle("invisible");
    ...
    setTimeout(() => {
      ...
      copyConfirmation?.classList.replace("opacity-100", "opacity-0");
      setTimeout(() => {
        copyConfirmation?.classList.toggle("invisible");
      }, copyConfirmationAnimDuration);
    }, 1000);
  };
  ...
  return (
    <>
      <div
        className={`invisible fixed top-7 left-[50%] flex w-max -translate-x-[50%] cursor-default items-center justify-around rounded-md bg-green-500/90 opacity-0 transition-opacity duration-${copyConfirmationAnimDuration}`}
        ref={copyConfirmationRef}
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 24 24"
          className="size-10 w-min fill-stone-200"
        >
          <path d="M21,7L9,19L3.5,13.5L4.91,12.09L9,16.17L19.59,5.59L21,7Z" />
        </svg>
        <p className="flex-1 p-2 text-xl font-bold">Email copied!</p>
      </div>
